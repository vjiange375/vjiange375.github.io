...
function fit() {
  ...
  const fontSizeBase = isMobile ? 300 : 500;
  ...
  // Expose fontSize and gap to letter()
  shapeCanvas.__gap = gap;
  shapeCanvas.__fontSize = fontSizeBase;
}

function processCanvas() {
  const gap = shapeCanvas.__gap;
...
return {
  ...
  letter: function (l) {
    const isMobile = window.innerWidth <= 640;
    const gap = shapeCanvas.__gap;
    const fontSizeBase = shapeCanvas.__fontSize;

    setFontSize(fontSizeBase);
    let s = Math.min(
      fontSizeBase,
      (shapeCanvas.width / (shapeContext.measureText(l).width || 1)) * (isMobile ? 0.6 : 0.8) * fontSizeBase,
      (shapeCanvas.height / fontSizeBase) * (isNumber(l) ? 1 : (isMobile ? 0.35 : 0.45)) * fontSizeBase
    );
    setFontSize(s);
    shapeContext.clearRect(0, 0, shapeCanvas.width, shapeCanvas.height);
